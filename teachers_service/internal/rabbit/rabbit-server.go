...
func StartRabbitServer() {
	connGrpc, err := grpc.Dial("localhost:8003", grpc.WithTransportCredentials(insecure.NewCredentials()))
	...
	client := pb.NewTeachersServiceClient(connGrpc)
	...
	go func() {
		...
		for d := range msgs {
			...
			res, err := client.GetTeacher(ctx, req)
